...
class RoundWoodPurchase(models.Model):
    ...
    def save(self, *args, **kwargs):
        """Calculate volume and total cost before saving"""
        # Calculate volume if not already done
        if not self.volume_cubic_feet or self.volume_cubic_feet == 0:
            self.volume_cubic_feet = self.calculate_volume()
        
        # Calculate total cost
        self.total_cost = self.volume_cubic_feet * self.unit_cost_per_cubic_foot
